...
//this socket always has id either for SIM or PHONE
	...
	Logger log;
	ArrayBlockingQueue<String> outBoundDataQ;
	ArrayBlockingQueue<String> urgentMailBox;
	...
	static final int normalHeartBeatInterval=60*1000;
	...
	public ArrayBlockingQueue<String> getUrgentMailBox(){
		return urgentMailBox;
	}

	public String readUrgentMail(){
		try {
			String urgentMail=urgentMailBox.poll(5 , TimeUnit.MINUTES);
			return urgentMail;
	...
	public void setUrgentMailBox(ArrayBlockingQueue<String> aBox){
		urgentMailBox=aBox;
	}
	/**
	 *
	...
	private void init() {
		...
		//if (outBoundDataQ == null)
			//outBoundDataQ = new ArrayBlockingQueue<String>(Q_SIZE, true);  should be assigned by postMaster
		//if (socketInDataQ == null)
			//socketInDataQ = new ArrayBlockingQueue<String>(Q_SIZE, true); should be assigned by postMaster
	...
	public void resetHeartBeatInterval(int interval) //in milliseconds
	{
		mHeartBeatInterval = interval;
	}
	...
	public void setOutBoundDataQ(ArrayBlockingQueue<String> newQ) {
		outBoundDataQ = newQ;
	}
	...
	public ArrayBlockingQueue<String> getOutBoundDataQ() {
		...
		//if (outBoundDataQ == null)
			//outBoundDataQ = new ArrayBlockingQueue<String>(Q_SIZE, true);
		return outBoundDataQ;
	}
	...
	{
		byte[] readData = null;
		//long timeEnd = (new Date()).getTime() + 10 * 1000;
		//while (readData == null || readData.length < 1) {
			if (mySocket == null || mySocket.isSktClosed()) {
				stopFlag = true;
				return;
			}
			...
			if (readData == null && !writeThread.isAlive()) {
				stopFlag = true;
				mySocket.close();
				return;
			}
			...
			if (msg.substring(0, 2).equalsIgnoreCase("S2") || msg.substring(0, 2).equalsIgnoreCase("S3") ||
					msg.substring(0, 2).equalsIgnoreCase("S9")) {
				notUrgent = true;
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						urgentMailBox.put(data);
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
				}
			}).start();*/
				((ConnectDaemonService)mContext).processUrgentMsg(msg);
				stopFlag=true;
				mHeartBeatInterval=normalHeartBeatInterval;
			}
	}
	boolean notUrgent;
	...
	void processUrgentMsg(String msg)
	{
	...
	public void sendDataToServer() {
		...
		if (toSend.size() > 0) {
			...
			for (int i = 0; i < toSend.size(); i++) {
				try {
					outBoundDataQ.put(toSend.get(i));
				} catch (InterruptedException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
	...
	static boolean hasCommand=true;
	...
	boolean imDone;
	int iSuccessful = 0;
	private static final ReentrantLock lockRead=new ReentrantLock(true);
	...
	public void startJob() {
		...
		//do {
			...
			//mySimIccId .charAt(0) != '-')
			//iCanStart=true;
			// in the future release, the logic here is not appropriate,
			// the deliveryee and addressee should be posted by the sender (use structure)
			do {
				...
				iCanStart=false;
				if (mySimIccId.charAt(0) != '-' &&
					myName.charAt(0) == '-'){
						myName = mSPF.getString(MainActivity.PENDING_NEW_PHONE1, "--");
					myNickName = mSPF.getString(MainActivity.PENDING_NEW_PHONE2, "--");//key, value);
				}
		...
		try {
			...
			startWriteThread();
			int iOpenTime=2;
			if (notUrgent) iOpenTime=2;

			long timeEnd = (new Date()).getTime() + iOpenTime * 1000;
		...
		/*
			log.info("Socket closed for " + mHeartBeatInterval/1000+ " seconds");
					//DateFormat.getTimeInstance().format(new Date()));
	...
	public void interrupt()
	{
		if (!hasCommand) {
			dumpStack();
			Log.w("TCPDAEMON", "interrupted from "+getName());
		}
		super.interrupt();
	}
			...
			for (int k = 0; k < aCursor.getColumnCount(); k++) {
				switch (aCursor.getType(k)) {
					...
					case Cursor.FIELD_TYPE_INTEGER:
						aC.put(heads[k], "" + aCursor.getInt(k));
						break;
					case Cursor.FIELD_TYPE_FLOAT:
						aC.put(heads[k], "" + aCursor.getFloat(k));
						break;
					case Cursor.FIELD_TYPE_STRING:
						aC.put(heads[k], aCursor.getString(k));
						break;
					case Cursor.FIELD_TYPE_BLOB:
						aC.put(heads[k], new String(aCursor.getBlob(k)));
						break;
					default:
						aC.put(heads[k], "0");
						break;
